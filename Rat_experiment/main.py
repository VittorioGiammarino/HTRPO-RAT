...
time = np.linspace(init_time,final_time, len(T_set[init:,0])) 
fig, ax = plt.subplots()
plot_data = plt.scatter(T_set[init:last_step,0], T_set[init:last_step,1], c=time[init:last_step], marker='o', cmap='cool') #-1], marker='o', cmap='cool')
cbar = fig.colorbar(plot_data, ticks=[10, 100, 200, 300, 600, 900])
cbar.ax.set_yticklabels(['time = 0', 'time = 100', 'time = 200', 'time = 300', 'time = 600', 'time = 900'])
plt.xlabel('x')
plt.ylabel('y')
plt.title('Cleaned data')
plt.savefig('Figures/cleaned_data_last_step{}.eps'.format(last_step), format='eps')
plt.show() 
